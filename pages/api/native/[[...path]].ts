...
const TARGET = process.env.NATIVE_OPENCLAW_URL || 'http://localhost:8080';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.body) {
    ...
    console.log(`[NATIVE] >> ${req.method} ${path} body: ${preview}`);
  } else {
    console.log(`[NATIVE] >> ${req.method} ${path}`);
  }
  ...
  return new Promise<void>((resolve) => {
    const proxyReq = lib.request(
      ...
      (proxyRes) => {
        console.log(`[NATIVE] << ${req.method} ${path} -> ${proxyRes.statusCode}`);
    ...
    proxyReq.on('error', (err) => {
      console.error(`[NATIVE] !! ${req.method} ${path} error: ${err.message}`);
